...
(function () {
  ...
  function renderProcess() {
    ...
    $('#processStatic').innerHTML = track.innerHTML;
    $('#processCount').textContent = '01 / ' + String(items.length).padStart(2, '0');
  }
  ...
  function initMotion() {
    ...
    // proceso: pin + scroll horizontal (solo pantallas grandes)
    gsap.matchMedia().add('(min-width: 900px)', function () {
      var track = $('#processTrack');
      var viewport = $('.process-viewport');
      if (!track || !viewport || track.scrollWidth <= viewport.clientWidth) return;
      var dist = function () { return track.scrollWidth - viewport.clientWidth; };
      var steps = $all('.process-step');
        ...
        scrollTrigger: {
          ...
          onUpdate: function (self) {
            ...
            if (count && steps.length) {
              var idx = Math.min(steps.length, Math.floor(self.progress * steps.length) + 1);
              count.textContent = String(idx).padStart(2, '0') + ' / ' + String(steps.length).padStart(2, '0');
            }
          }
        }
      });
